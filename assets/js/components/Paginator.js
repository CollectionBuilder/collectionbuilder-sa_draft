...
import { createElement, getAttribute } from "../helpers.js"


/******************************************************************************
 * Page Button Component
 *
 * This component implements a single page link.
 *
 ******************************************************************************/

class PageButton extends HTMLButtonElement {
...
customElements.define("page-button", PageButton, { extends: "button" })
...
export default class Paginator extends HTMLElement {
  connectedCallback () {
    ...
    const numTotal = parseInt(getAttribute(this, "num-total"))
    const pageSize = parseInt(getAttribute(this, "page-size"))
    const currentPage = parseInt(getAttribute(this, "current-page"))
    ...
    this.appendChild(new PageButton("&laquo;&nbsp;prev", pageToStart(currentPage - 1),
                                    currentPage === 1))

    // Maybe add a first page button.
    if (showFirstPageButton) {
      this.appendChild(new PageButton("1", 1, false))
    ...
    while (pageDelta <= MAX_NUM_REACHABLE_PAGES) {
      ...
      if (
        ...
        this.appendChild(
          new PageButton(`${page}`, pageToStart(page), isCurrentPage, isCurrentPage)
        )
      }
      pageDelta += 1
    ...
    if (showLastPageButton) {
      this.appendChild(ellipsisElement.cloneNode(true))
      this.appendChild(new PageButton(`${maxPage}`, pageToStart(maxPage), false))
    }

    // Add the next page button.
    const nextPageButton = new PageButton("next&nbsp;&raquo;",
                                          pageToStart(currentPage + 1),
                                          currentPage === maxPage)
